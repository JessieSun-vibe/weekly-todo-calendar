...
export const createDeleteLinesHandler =
  (props: {
  ...
  async (target: {
    ...
    await props.vaultFacade.editFile(target.path, (contents) => {
      const lines = contents.split("\n");
      const startLine = target.position.start.line;
      const endLine = Math.max(startLine, target.position.end.line);

      if (startLine < 0 || startLine >= lines.length) {
        throw new Error(`Task line ${startLine + 1} no longer exists`);
      }
