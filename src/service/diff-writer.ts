...
function mapTaskDiffToUpdate(props: {
  ...
  if (type === "added") {
    ...
    if (weeklyFilePath) {
      const scheduledDate = task.startTime.format("YYYY-MM-DD");
      let contents = scheduledPropRegExps.some((regexp) =>
        regexp.test(taskTextWithUpdatedProps),
      )
        ? taskTextWithUpdatedProps
        : `${taskTextWithUpdatedProps} [scheduled:: ${scheduledDate}]`;
      if (!/\[calendar-id::\s*[A-Za-z0-9_-]+\]/.test(contents)) {
        const id = `wtc_${crypto.randomUUID().replaceAll("-", "")}`;
        contents += ` [calendar-id:: ${id}]`;
      }
