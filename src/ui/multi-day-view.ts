...
export default class MultiDayView extends ItemView {
  ...
  async onOpen() {
    ...
    ).__obsidianWeeklyCalendarWeekStart;
    delete (
      window as typeof window & {
        __obsidianWeeklyCalendarWeekStart?: string;
      }
    ).__obsidianWeeklyCalendarWeekStart;
    const requestedStart = requestedWeekStart
      ? window.moment(requestedWeekStart, "YYYY-MM-DD", true)
      : undefined;

    const range =
      requestedStart?.isValid() && currentSettings.multiDayRange === "full-week"
        ? r.getFullWeek(requestedStart, currentSettings.firstDayOfWeek)
        : r.createRange(
