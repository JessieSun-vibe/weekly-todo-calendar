...
export default class DayPlanner extends Plugin {
  ...
  async onload() {
    ...
    new WeeklyTodoFeature(this, this.app, this.settings).load();
  ...
  private registerViews(props: {
    ...
    const deleteLines = createDeleteLinesHandler({
      vaultFacade: this.vaultFacade,
      onConfirmed: (path) => {
        // Remove the old file snapshot immediately so deleted blocks disappear
        // without waiting for Obsidian's asynchronous metadata-cache refresh.
        dispatch(fileDeleted({ path }));
        window.setTimeout(() => dispatch(indexRequested([path])), 100);
        this.undoNotice.show();
      },
    });
